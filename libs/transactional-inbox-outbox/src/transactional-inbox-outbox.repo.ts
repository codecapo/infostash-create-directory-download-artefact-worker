...
export class TransactionalInboxOutboxRepo {
  ...
  constructor(
    @InjectModel(InboxProcessingLog.name)
    private inboxProcessingLogModel: Model<InboxProcessingLogDocument>,
    @InjectModel(OutboxProcessingLog.name)
    private outboxProcessingLogModel: Model<OutboxProcessingLogDocument>,
  ) {}
  ...
  public async updateProcessingLogState(
    ...
    const update = await this.inboxProcessingLogModel
      .findOneAndUpdate(
        { _id: processingLogRequestOid },
        {
  ...
  public async isInboxProcessingLogIsPresentInOutboxProcessingLogAndAllStatesArePresentInProcessingStage(
    inboxProcessingLogRequestId: string,
  ) {
    const outboxItems = await this.outboxProcessingLogModel.find({
      inboxProcessingLogRequestId: new Types.ObjectId(
        inboxProcessingLogRequestId,
      ),
    });
    if (outboxItems.length < 1) {
      this.logger.debug(
        `Got new processing log request with id: ${inboxProcessingLogRequestId}`,
      );
      return true;
    } else {
